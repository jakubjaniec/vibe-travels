...
setup('authenticate', async ({ page, baseURL }) => {
  ...
  const emailInput = page.locator('input[data-testid="login-email-input"]');
  ...
  const passwordInput = page.locator('input[data-testid="login-password-input"]');
  ...
  await expect(page.getByRole('heading', { name: 'Vibe Travels', level: 2 })).toBeVisible({
    timeout: 10000,
  });
